 ...
 */
VDom.prototype.render = function (vNode = this.virtualize(this.$template)) {
  if (vNode.text) {
...
VDom.prototype.renderElem = function (vEl) {
  ...
  const { tagName, attrs, children, anchor } = vEl;
  let $el;
  ...
  if (anchor == null) {
    ...
    $el = this.render({ text: true, content: anchor });
  }
  ...
  this.elDirectives.push(...directives);
  return $el;
};
